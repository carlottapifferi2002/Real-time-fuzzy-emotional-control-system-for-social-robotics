...
class FuzzyNode:
    ...
    def expression_callback(self, msg):
        ...
        if expr_str not in EXPRESSION_MAP:
            rospy.logwarn(f"Expression '{expr_str}' not recognised.")
            return
        face_value = EXPRESSION_MAP[expr_str]

        if face_value == self.last_face_value:
            # The face is the same as the previous one: skip recomputation and return
            return
        self.last_face_value = face_value

        # Choose the current velocity to use as input for the fuzzy controller:
        ...
        # Store the latest variables so they can always be displayed in the loop
        ...
        rospy.loginfo(f"Detected expression: {expr_str} (valore: {face_value})")
        ...
        try:
            ...
            # Mapping of tuning_label to max_vel_theta and security_dist
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            rospy.loginfo(
                f"Current base velocity: {self.current_vel_x:.3f} m/s"
            )
            rate.sleep()
